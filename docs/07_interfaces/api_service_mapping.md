...
| API-SETTINGS-PROVIDERS-ENABLEMENT-STATUS | `GET /api/v1/settings/providers/enablement` | `settings.providers.enablement.status` | `SettingsApplicationService -> ProviderEnablementService` | read-only redacted runtime state | human session + RBAC + PolicyEngine |
| API-SETTINGS-PROVIDERS-ENABLEMENT-PLAN | `POST /api/v1/settings/providers/enablement/plan` | `settings.providers.enablement.plan` | `SettingsApplicationService -> ProviderEnablementService` | plan-only, no write | human session + owner RBAC + PolicyEngine |
| API-SETTINGS-PROVIDERS-CONNECTIVITY | `POST /api/v1/settings/providers/connectivity-test` | `settings.providers.connectivity_test` | `SettingsApplicationService -> ProviderEnablementService` | hermetic/fake connectivity, no external network | human session + owner RBAC + PolicyEngine |
| API-SETTINGS-PROVIDERS-ENABLEMENT-APPLY | `POST /api/v1/settings/providers/enablement` | `settings.providers.enablement.apply` | `SettingsApplicationService -> ProviderEnablementService` | ignored runtime state write, approval-bound | human session + owner RBAC + approval + PolicyEngine |
| API-SETTINGS-PROVIDERS-DISABLE | `POST /api/v1/settings/providers/disable` | `settings.providers.enablement.disable` | `SettingsApplicationService -> ProviderEnablementService` | runtime kill switch only | human session + owner RBAC + PolicyEngine |
...
| GET | `/api/v1/auth/capabilities` | `auth.capabilities` / `API-GSDLC02C-RBAC-CAPABILITIES` | `ApplicationService.rbac` / `RBACApplicationService` | human-session + server RBAC + PolicyEngine | sanitized capability view; frontend is not authority |
...
## DEVPL-GSDLC-04 artifact workbench current-active mapping

Successor mapping for governed artifact authoring/import/review. Historical API mappings remain preserved above; these routes use local human-session authority and never expose arbitrary shell/network execution.

| Method | Path | Operation | API ID | Application/domain service | Policy/gate |
|---|---|---|---|---|---|
| `GET` | `/api/v1/workspace/artifact-drafts/{document_id}` | `workspace.artifact_drafts.get` | `API-GSDLC04B-ARTIFACT-DRAFT-GET` | `ApplicationService -> ArtifactDraftApplicationService` | Human session + CSRF when mutating; local-only; no arbitrary shell/network/external API; source mutation=false |
| `GET` | `/api/v1/workspace/artifact-drafts/{document_id}/history` | `workspace.artifact_drafts.history` | `API-GSDLC04B-ARTIFACT-DRAFT-HISTORY` | `ApplicationService -> ArtifactDraftApplicationService` | Human session + CSRF when mutating; local-only; no arbitrary shell/network/external API; source mutation=false |
| `POST` | `/api/v1/workspace/artifact-drafts/{document_id}/save` | `workspace.artifact_drafts.save` | `API-GSDLC04B-ARTIFACT-DRAFT-SAVE` | `ApplicationService -> ArtifactDraftApplicationService` | Human session + CSRF when mutating; local-only; no arbitrary shell/network/external API; source mutation=false |
| `POST` | `/api/v1/workspace/artifact-drafts/{document_id}/discard` | `workspace.artifact_drafts.discard` | `API-GSDLC04B-ARTIFACT-DRAFT-DISCARD` | `ApplicationService -> ArtifactDraftApplicationService` | Human session + CSRF when mutating; local-only; no arbitrary shell/network/external API; source mutation=false |
| `POST` | `/api/v1/workspace/artifact-drafts/{document_id}/recover` | `workspace.artifact_drafts.recover` | `API-GSDLC04B-ARTIFACT-DRAFT-RECOVER` | `ApplicationService -> ArtifactDraftApplicationService` | Human session + CSRF when mutating; local-only; no arbitrary shell/network/external API; source mutation=false |
| `POST` | `/api/v1/workspace/artifact-imports/preview` | `workspace.artifact_imports.preview` | `API-GSDLC04C-ARTIFACT-IMPORT-PREVIEW` | `ApplicationService -> ArtifactImportApplicationService` | Human session + CSRF when mutating; local-only; no arbitrary shell/network/external API; source mutation=false |
| `POST` | `/api/v1/workspace/artifact-imports/persist` | `workspace.artifact_imports.persist` | `API-GSDLC04C-ARTIFACT-IMPORT-PERSIST` | `ApplicationService -> ArtifactImportApplicationService` | Human session + CSRF when mutating; local-only; no arbitrary shell/network/external API; source mutation=false |
| `GET` | `/api/v1/workspace/artifact-imports/recent` | `workspace.artifact_imports.recent` | `API-GSDLC04C-ARTIFACT-IMPORT-RECENT` | `ApplicationService -> ArtifactImportApplicationService` | Human session + CSRF when mutating; local-only; no arbitrary shell/network/external API; source mutation=false |
| `POST` | `/api/v1/workspace/artifact-reviews/imports/{import_id}/start` | `workspace.artifact_reviews.start_import` | `API-GSDLC04D-ARTIFACT-REVIEW-START-IMPORT` | `ApplicationService -> ArtifactReviewApplicationService` | Human session + CSRF when mutating; local-only; no arbitrary shell/network/external API; source mutation=false |
| `POST` | `/api/v1/workspace/artifact-reviews/documents/{document_id}/start` | `workspace.artifact_reviews.start_document` | `API-GSDLC04D-ARTIFACT-REVIEW-START-DOCUMENT` | `ApplicationService -> ArtifactReviewApplicationService` | Human session + CSRF when mutating; local-only; no arbitrary shell/network/external API; source mutation=false |
| `GET` | `/api/v1/workspace/artifact-reviews/{review_id}` | `workspace.artifact_reviews.status` | `API-GSDLC04D-ARTIFACT-REVIEW-STATUS` | `ApplicationService -> ArtifactReviewApplicationService` | Human session + CSRF when mutating; local-only; no arbitrary shell/network/external API; source mutation=false |
| `POST` | `/api/v1/workspace/artifact-reviews/{review_id}/freeze` | `workspace.artifact_reviews.freeze` | `API-GSDLC04D-ARTIFACT-REVIEW-FREEZE` | `ApplicationService -> ArtifactReviewApplicationService` | Human session + CSRF when mutating; local-only; no arbitrary shell/network/external API; source mutation=false |
