...
La ejecución Windows autoritativa levantó API y UI en `127.0.0.1`, verificó token temporal no persistente, autenticación `401/401/200`, CORS local GET/preflight, rechazo de origen no confiable, smokes npm/API/UI, ausencia de secretos y liberación de procesos/puertos. Evidencia final SHA-256: `c962739b1c9f9045ea872be9b576f6045aa41268261b1aab5bc3ae629824d8a5`; evidencia prioritaria SHA-256: `4c5596d09c4208ccd092f42f110e8b23609b1d4de98166140fc26ac9b95407c5`.

Gap S3 no bloqueante: el OpenAPI estático omite cuatro rutas públicas de transporte aunque runtime/canonical/registry permanecen alineados y todas las rutas protegidas conservan auth/policy. `POST-H-EVAL-002-01-D` queda autorizado para aceptación visual formal, matriz de estados negativos, screenshots y bridge register. Un smoke manual exploratorio puede ejecutarse antes de 01-D, pero no sustituye su evidencia.
...
POST-H-020-B agrega validación semántica local para los mappings de compliance. El validator no recolecta evidencias, no genera reportes finales y no ejecuta comandos declarados en los mappings; verifica consistencia, unicidad, cobertura mínima por dominio y ausencia de claims prohibidos.
...
PASS si todos los `control_id`/`evidence_id` son únicos, cada `required_evidence` tiene mapping, ningún control crítico queda sin evidencia, la cobertura incluye `security`, `testing`, `policy`, `release`, `observability` y `agentic`, y `certification_claimed=false` / `legal_advice_claimed=false`.

BLOCK si se declara certificación, asesoría legal, evidencia requerida sin mapping, duplicados, source commands externos o mutantes, red, APIs externas, remediación automática o envío de evidencias a terceros.

Límite: esta sigue siendo una versión local, declarativa y no-certificante. El collector, report generator, CLI `compliance mapping report`, audit pack integration y quality gate quedan para POST-H-020-C/D/E.

Último hito cerrado: `POST-H-019 — Plugin sandbox design sin ejecución arbitraria`; hito activo: `POST-H-020 — Compliance mapping packs ampliados`; último micro-sprint implementado: `POST-H-020-B — Compliance mapping validator`; siguiente micro-sprint: `POST-H-020-C — Evidence collector y report generator local`.
...
## FRX-v2.4-B — Current Execution Profile Lock and Windows Adoption (2026-09-06)

FRX-v2.4-B centralizes Full Regression execution policy under the versioned current profile `frx-v2.4-current`. Backlog operators consume a profile id and high-level state only; planner, shard size, max nodeids, nodeid transport and worker policy are not consumer knobs. `tests full-regression preflight` is read-only and must PASS before a legitimate later backlog can reserve its one-full budget. It validates the sealed collection, full Isolation/Duration schemas and coverage, conflict/isolation consistency, cold-start treatment, complete-plan ETA, projected shard count, worker policy and current budget state. Do not pass an invented environment label such as `windows`; omit `--environment` so the current profile supplies the canonical `environment_fingerprint`, unless an approved successor profile explicitly changes it. The GSDLC-08-E count50 topology fixture must BLOCK before budget reservation; the current v2.3-derived fixture must PASS. Direct Quality/UOC full workers remain blocked even with approval: use the governed FRX full-session path. FRX-v2.4-B itself executes no full regression and no browser; API/UI are not started. Windows installation, Git three-state promotion, evidence capture and packaging are governed exclusively by the FRX-v2.4-B bundle guide.

## DEVPL-GSDLC-09 activation/rebind (2026-09-06)

The activation is an administrative, fail-closed predecessor of GSDLC-09-A. It binds repo406 and the current FRX execution profile without changing `src/`. Validate Project State, TCR v1/v2, Documentation Governance, Evidence Freshness, API contract drift, Historical Contract Authority and Contract Reconciliation before Windows close. Do not run Full Regression or browser. After Windows PASS, the activation successor authorizes 09-A; low-level FRX planner/max-nodeids/transport/workers never belong to the GSDLC operator. Text pre/postimage comparisons use semantic UTF-8/LF normalization so CRLF cannot create a false BLOCK.
## DEVPL-GSDLC-09-A — StoryExecutionState and context pack (2026-09-06)
