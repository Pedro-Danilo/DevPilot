...
SCRIPT_ID="DEVPL-GSDLC-04-C-RUNTIME-CONSOLE"
VERSION="1.0.0"
...
def validate_fixture(raw:str)->Path:
    ...
    if f!=expected: raise RuntimeError(f'Fixture 04-C no autorizado: {f}; esperado exactamente {expected}.')
    if not f.is_dir() or not (f/'.git').exists(): raise RuntimeError('Fixture 04-C no existe o no es Git; ejecute prepare-browser desde Consola 1.')
    ...
    if 'inventory-sales-local' in str(f).lower() or 'devpilot_workspaces' in str(f).lower(): raise RuntimeError('Workspace piloto real prohibido en 04-C.')
...
def main()->int:
    p=argparse.ArgumentParser(description='Dedicated 04-C API/UI launcher. Use separate Console 2/3.')
    ...
    try:
        ...
        if role=='api':
            ...
            binding={'allowed_workspace_root':str(fixture),'active_workspace_root':str(fixture),'registry_env_cleared':True,'scope':'gsdlc-04-c-browser-fixture-only'}
