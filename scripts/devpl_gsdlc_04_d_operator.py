...
OPERATOR_ID = "DEVPL-GSDLC-04-D-OPERATOR"
OPERATOR_VERSION = "1.0.0"
BASELINE_REPO = "repo_DevPilot_Local_367_DEVPL_GSDLC_04_C_EXTERNAL_SOURCE_IMPORT_WINDOWS_VALIDATED_CANDIDATE.zip"
BASELINE_COMMIT = "ce03b2975320617e8a3663ced2d15736aa9e3c1a"
BASELINE_SHA256 = "7700f77d00d578c183cd47908996235cc898d49876c8d48278b21c0b905d8484"
DEFAULT_BRANCH = "feat/devpl-gsdlc-04-d-governed-artifact-apply"
DEFAULT_CANDIDATE = "repo_DevPilot_Local_368_DEVPL_GSDLC_04_D_GOVERNED_ARTIFACT_APPLY_WINDOWS_VALIDATED_CANDIDATE.zip"
BROWSER_MARKER = "DEVPL_GSDLC_04_D_BROWSER_ACCEPTANCE_VALIDATION.json"
...
def prepare_repo(repo: Path, branch_name: str) -> dict[str, Any]:
    ...
    if git_status(repo):
        raise OperatorBlock("Repo must be clean before 04-D branch preparation; no reset/clean is authorized.")
    if head(repo) != BASELINE_COMMIT:
        raise OperatorBlock(f"HEAD must be 04-C predecessor {BASELINE_COMMIT}; actual={head(repo)}")
    ...
    if probe.returncode == 0:
        ...
        raise OperatorBlock("git show-ref failed while inspecting 04-D branch.")
...
def preflight(repo: Path, package_root: Path, manifest: dict[str, Any], branch_name: str) -> dict[str, Any]:
    if head(repo) != BASELINE_COMMIT or branch(repo) != branch_name:
        raise OperatorBlock(f"04-D preflight requires HEAD={BASELINE_COMMIT} on {branch_name}; actual={head(repo)} {branch(repo)}")
    allowed = {x["path"] for x in manifest["files"]}
    unknown = [entry for entry in git_status(repo) if dirty_path(entry) not in allowed]
    if unknown:
        raise OperatorBlock(f"Unknown dirty paths outside governed 04-D delta: {unknown[:20]}")
    result = classify(repo, package_root, manifest)
    if result["conflicts"]:
        raise OperatorBlock(f"04-D preflight conflicts: {result['conflicts'][:12]}")
...
def replace_with_retry(src: Path, dst: Path, attempts: int = 5) -> None:
    ...
    temp = dst.with_name(dst.name + ".devpilot04d.tmp")
    if temp.exists():
        temp.unlink()
...
def repo_review(repo: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if unexpected:
        raise OperatorBlock(f"Unexpected Git paths outside 04-D source delta: {unexpected[:20]}")
    cp = git(repo, "-c", "core.safecrlf=false", "diff", "--check", check=False)
    real_errors = [ln for ln in cp.stdout.splitlines() if ln.strip() and "LF will be replaced by CRLF" not in ln and "CRLF will be replaced by LF" not in ln and "original line endings" not in ln]
    if real_errors:
...
def validate(repo: Path, evidence: Path) -> dict[str, Any]:
    ...
    gates: list[tuple[str, list[str], int]] = [
        ("focused-04d", [py, "-m", "pytest", "--assert=plain", "-q", "tests/test_devpl_gsdlc_04_d_artifact_review_apply_freeze.py"], 300),
        ...
        ("planner", [py, "-m", "pytest", "--assert=plain", "-q", "tests/test_workspace_edit_plan_service.py"], 300),
        ("executor", [py, "-m", "pytest", "--assert=plain", "-q", "tests/test_workspace_edit_execution_service.py"], 300),
        ("api-plan-execution", [py, "-m", "pytest", "--assert=plain", "-q", "tests/test_api_workspace_edit_plans.py", "tests/test_api_workspace_edit_execution.py"], 300),
        ...
        ("ui-static-04d", [npm, "--prefix", "ui/web", "run", "test:artifact-review"], 180),
    ...
    return {"status": "PASS", "gates": out, "validation_scope": "GSDLC-04-D focal + cumulative A-D + Test Impact + deterministic governance/API/UI", "full_regression_runs": 0, "full_regression_executed": False, "network_used": False, "external_api_used": False, "pilot_workspace_accessed": False}
...
def git_commit(repo: Path, manifest: dict[str, Any], evidence: Path, message: str) -> dict[str, Any]:
    ...
    if not marker.is_file() or json.loads(marker.read_text(encoding="utf-8")).get("status") != "PASS":
        raise OperatorBlock("Browser evidence PASS marker missing; 04-D commit is blocked.")
    review = repo_review(repo, manifest)
    dirty = sorted({dirty_path(x) for x in review["status_entries"]})
    if not dirty:
        raise OperatorBlock("No 04-D source changes are dirty; refusing empty commit.")
    allowed = {x["path"] for x in manifest["files"]}
    if not set(dirty) <= allowed:
        raise OperatorBlock("Dirty paths exceed governed 04-D delta.")
    for rel in dirty:
        git(repo, "add", "--", rel)
    ...
    if git_status(repo):
        raise OperatorBlock("Worktree not clean after 04-D commit.")
...
def package_head(repo: Path, artifacts_root: Path, name: str) -> dict[str, Any]:
    ...
    outdir = artifacts_root / "baselines" / "gsdlc_04_d"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Governed GSDLC-04-D source operator; state-aware and dry-run by default.")
    ...
    parser.add_argument("--commit-message", default="feat(gsdlc-04-d): govern artifact review apply and freeze")
    ...
    try:
        ...
        if "inventory-sales-local" in low or "devpilot_workspaces" in low:
            raise OperatorBlock("Pilot workspace access forbidden during GSDLC-04-D.")
        if not repo.is_dir():
