...
FRX-v2.4-B is implemented and `LOCAL-QUALIFIED/PENDING-WINDOWS` on the Windows-validated repo405 successor. Full Regression consumers are now profile-id-only: the current profile `frx-v2.4-current` locks deterministic LPT, 900 s target shards, max 200 nodeids, manifest transport, workers=1 by default, safe parallel <=2 only by prior opt-in, unknown/unclassified serial, completion-first, exact accounting, one-full/resume/composite-recovery semantics and semantic source guards. A read-only preflight validates sealed collection, complete Isolation/Duration schema+coverage, conflict/isolation consistency, full-plan ETA, topology, worker policy and budget before any full reservation. The historical GSDLC-08-E `count50/max50/command-line` topology is a negative fixture and BLOCKs before budget reservation. Direct UOC full workers are blocked. Local qualification is focal 18/18 + bounded impact 39/39 + deterministic gates/guard PASS; full=0, browser=0, API/UI not started. DEVPL-FRX-v2.4 and GSDLC-09 are not closed/activated until the supplied Windows operator passes.
