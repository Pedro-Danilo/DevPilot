...
## DEVPL-GSDLC-11-B — Reproducible package, checksum and SBOM — local qualification candidate

GSDLC-11-B is implemented on Windows-validated `repo421` (`01c28e73994b74699802dcbac9bb06d686841b89`, SHA-256 `e105736c539f37ea3ad81b96ad571149035c5bb77602e0303018f304c04e8955`). The release workbench composes the existing local packaging stack as typed operations: dry-run plan -> exact Git commit/tree revalidation -> deterministic source ZIP -> forbidden-entry/secret policy -> SHA-256 sidecars -> CycloneDX-compatible SBOM baseline -> release/artifact manifests -> reproducibility verification. No arbitrary shell, publish, upload, signing or deploy is introduced. Browser UI exposes package status/plan/execute while server session/RBAC/project context remains authority. Full Regression = 0; GSDLC-11 budget remains 0/1 for 11-E.

## DEVPL-GSDLC-10-A — Test Impact + StoryTestPlan UI-native — local qualification candidate

GSDLC-10 activation/rebind is folded into 10-A on cumulative execution source `repo_DevPilot_Local_414_DEVPL_GSDLC_09_FINAL_AUTHORITY_POINTERS_WINDOWS_VALIDATED_CANDIDATE.zip`; repo413 remains the canonical functional predecessor baseline. Story Code Workbench now exposes `Story → Validar → Test Impact v2 → StoryTestPlan review/approval` without a second Test Impact engine or Quality UI. StoryTestPlan is bound to StoryExecution + immutable SourceChangePlan hash, explains required/recommended tests, escalates unknown/sensitive impact fail-closed, keeps waivers human-owner/server-side with expiry, and exposes Full Regression only as a non-executable signal (`10-A full=0`). Windows live browser remains required once because the visible Validate journey changed.
...
## DEVPL-GSDLC-09 — activation/rebind to repo406 — Windows closure

The mandatory pre-functional activation is implemented as governance-only rebind from `repo_DevPilot_Local_406_FRX_V2_4_B_EXECUTION_PROFILE_LOCK_WINDOWS_VALIDATED_CANDIDATE.zip` (`6b8a9a5feef65860826904444f651421abad282a`, SHA-256 `59c40713182b84655315773654e49adf95419ceb517ec9496929252fba33b191`). FRX-v2.4 remains `CLOSED/PASS/WINDOWS-VALIDATED`, and GSDLC-09 binds the current profile `frx-v2.4-current` by profile id/hash only. Activation changes Project State/source authority/documentation but performs no functional `src/` mutation, no Full Regression, no browser, no API/UI runtime and no network/external API. Activation closure produces `repo_DevPilot_Local_407_DEVPL_GSDLC_09_ACTIVATION_REBIND_WINDOWS_VALIDATED_CANDIDATE.zip` and authorizes GSDLC-09-A; 09-A functionality is still not implemented by this activation.

## FRX-v2.4-B — Current Execution Profile Lock and Windows Adoption — local qualification candidate

FRX-v2.4-B is `CLOSED/PASS/WINDOWS-VALIDATED` after the profile-locked Windows operator confirmed focal 18/18, bounded impact 39/39, current-profile/preflight/topology fixtures and deterministic gates without consuming a Full Regression or browser run. Full Regression consumers are profile-id-only under `frx-v2.4-current`; the GSDLC-08-E `count50/max50/command-line` topology BLOCKs before budget reservation, direct UOC full workers remain blocked, and the representative sealed 2883-nodeid preflight preserves 2804 known durations + 79 cold-starts, 15 projected shards and workers=1 with `budget_reserved=false`. `DEVPL-FRX-v2.4` is therefore closed and only the activation/rebind of GSDLC-09 is authorized; GSDLC-09 itself is not implemented by this closure. Full=0, browser=0 and API/UI are not started by this enabler.
...
### DEVPL-GSDLC-10-D — Windows closure

Estado: `CLOSED/PASS/WINDOWS-VALIDATED`. Stage/commit story-bound exacto con RBAC y approvals separados fue demostrado desde UI sobre fixture Git aislado; traceability completa, worktree clean, push/force/rebase/reset-hard=false, S0/S1=0 y Full Regression=0. Successor: `repo_DevPilot_Local_419_DEVPL_GSDLC_10_D_RBAC_GOVERNED_STAGE_COMMIT_WINDOWS_VALIDATED_CANDIDATE.zip`. Próximo micro-sprint autorizado: `DEVPL-GSDLC-10-E`.
...
## DEVPL-GSDLC-12-A — activation/rebind + persistent recovery local qualification

DEVPL-GSDLC-12 is current-active on Windows-validated repo425 (`b341370633e66355add6bb2611879b32f277ad0f`, SHA-256 `d4a9cf4b3b8d544b71ee9b568e7ba3388cf39e74ce8d213d3b0e9f9d2946695d`). Activation/rebind is absorbed into 12-A. The implementation adds durable metadata-only `ResumeService`, exact owner/session `WorkspaceLockService`, project-scoped `RecoveryView`, server-side restart recovery, stale-lock explicit recovery, source/approval revalidation and post-release lifecycle reconciliation without granting browser storage authority or auto-resuming sensitive mutations. Runtime DBs are excluded from recovery authority. Status: `IMPLEMENTED/LOCAL-QUALIFIED/WINDOWS-VALIDATION-PENDING`; S0/S1=0 locally; Full Regression=0 and DEVPL-GSDLC-12 budget remains 0/1 reserved for 12-E. GSDLC-12-B is not authorized until Windows browser/restart acceptance closes 12-A.
### DEVPL-GSDLC-12-A — Windows closure
...
## DEVPL-UX-P0-B — cierre final Windows con corrective — 2026-09-15

`CLOSED/PASS/WINDOWS-VALIDATED`. El candidate inicial repo432 pasó la automatización pero su evidencia visual reveló un mismatch entre el nombre físico del active root (`DevPilot_Local`) y la identidad lógica autoritativa (`devpilot-local`). El correctivo deriva ahora el workspace lógico desde `.devpilot/project.yaml` (`project.id`), mantiene PathGuard/RBAC invariantes y preserva Project Status aunque una proyección auxiliar falle. Browser corrective PASS demuestra Project Status, Recovery y Reconciliation autorizados; Guided/Expert parity y viewport móvil PASS. Full Regression de UX-P0-B = 0. Successor autoritativo: `repo_DevPilot_Local_433_DEVPL_UX_P0_B_PROJECT_CONTEXT_AUTH_SCOPE_CORRECTIVE_WINDOWS_VALIDATED_CANDIDATE.zip` (`dc63672f2d617968998f3c68374a03581b348578`, SHA-256 `f4415775bd3bf5a01b6368197d0754374de93b659fa5f6bbff8b7a2b8ead4246`).

## DEVPL-UX-P0-C — Greenfield critical-path productization — 2026-09-15

Baseline: `repo_DevPilot_Local_433_DEVPL_UX_P0_B_PROJECT_CONTEXT_AUTH_SCOPE_CORRECTIVE_WINDOWS_VALIDATED_CANDIDATE.zip` / `dc63672f2d617968998f3c68374a03581b348578` / `f4415775bd3bf5a01b6368197d0754374de93b659fa5f6bbff8b7a2b8ead4246`. UX-P0-C productiza Home → Create/Open/Import → Project Status → Pre-code/Documents → Planning con una guía visual común, action hierarchy, progressive technical disclosure y un camino explícito para retomar contexto server-active. Routes, API contracts, RBAC, approvals y authority server-side permanecen invariantes. Esta es una primera productización bounded del critical path, no polish final: UX-P0-D/E continúan normalización cross-surface y cierre pre-pilot. Full Regression permanece 0/1 y está reservada exclusivamente para UX-P0-E. Successor esperado: `repo_DevPilot_Local_434_DEVPL_UX_P0_C_GREENFIELD_CRITICAL_PATH_WINDOWS_VALIDATED_CANDIDATE.zip`.
