...
class QualityGate:
    ...
    def _subgates(self) -> list[QualitySubgate]:
        ...
        if self.options.profile in {"ci", "release"}:
            ...
            subgates.append(QualitySubgate("advanced-evals-safety", "Advanced agentic, red-team, plugin ecosystem, multiworkspace, identity/RBAC and audit-pack safety eval suites meet scoring thresholds.", self._advanced_evals_safety))
    ...
    def _advanced_evals_safety(self) -> CommandResult:
        ...
        suites = ["advanced-agentic", "red-team", "plugin-ecosystem", "multiworkspace-isolation", "identity-rbac", "audit-pack-integrity"]
        ...
        return CommandResult(
            ...
            findings=[Finding("ADVANCED_EVALS_SAFETY_PASS", "Advanced agentic/red-team/plugin ecosystem/multiworkspace/identity/audit-pack eval suites meet safety thresholds.", Severity.INFO, metadata=summary)] if ok else blocking_findings,
        )
