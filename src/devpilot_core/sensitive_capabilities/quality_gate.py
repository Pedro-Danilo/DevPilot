...
from devpilot_core.sensitive_capabilities.validator import ConnectorWriteAdrValidator, PluginExecutionAdrValidator, RemoteExecutionAdr3Validator, MultiuserAuthAdrValidator
...
class SensitiveCapabilityAdrGate:
    ...
    to plugin.execution. POST-H-034-C adds remote.execution ADR-3. POST-H-034-D adds multiuser.auth. The gate
    ...
    def run(self) -> CommandResult:
        ...
        findings: list[Finding] = [*connector_result.findings, *plugin_result.findings, *remote_result.findings, *multiuser_result.findings]
        ...
        summary = {
            **connector_summary,
            "subgates_total": 4,
            "subgates_passed": int(connector_result.ok) + int(plugin_result.ok) + int(remote_result.ok) + int(multiuser_result.ok),
        ...
        return CommandResult(
            ...
            data={
                ...
                "notes": [
                    "POST-H-034-A/B/C/D gate validates ADR/no-go decisions only.",
                    "The gate does not enable connector write, plugin execution, remote execution, multiuser auth, external APIs, network use or source mutations.",
                ],
            },
            findings=findings,
        )
