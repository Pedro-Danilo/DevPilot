...
DECLARATIVE_GROUPS: dict[str, DeclarativeGroupDescriptor] = {
    ...
    "model": DeclarativeGroupDescriptor(
        group_id="model",
        domain="agentic.modeling",
        owner_module="src/devpilot_core/modeling/local_provider_health.py",
        application_service_required=True,
        recommended_tests=("python -m pytest -p no:ddtrace --assert=plain tests/test_post_h_032_local_llm_provider_hardening.py tests/test_model_adapter.py tests/test_model_governance.py tests/test_provider_config_schema.py tests/test_ollama_adapter.py tests/test_lmstudio_adapter.py tests/test_policy_engine.py -q",),
        rationale="POST-H-032-B governs ModelAdapter local provider hardening for Ollama/LM Studio; providers remain disabled by default, localhost-only, no-secret, fake-testable and external APIs stay blocked.",
    ),
...
COMMAND_OVERRIDES: dict[str, DeclarativeCommandOverride] = {
    ...
    "agent.rag-context": DeclarativeCommandOverride(
        command_id="agent.rag-context",
    ...
    "model.local-health": DeclarativeCommandOverride(
        ...
        policy_check_required=True,
        recommended_tests=(
            "python -m pytest -p no:ddtrace --assert=plain tests/test_post_h_032_local_llm_provider_hardening.py -q",
        ),
        rationale="POST-H-032-B builds local LLM provider hardening evidence without requiring real Ollama/LM Studio servers; optional reports are constrained to outputs/ and external APIs remain blocked.",
    ),
    ...
    "tests.full-session.collect": DeclarativeCommandOverride(
        command_id="tests.full-session.collect",
        risk_level=CommandRiskLevel.MEDIUM,
        side_effects=(CommandSideEffect.EXECUTE_SUBPROCESS, CommandSideEffect.WRITE_FILES),
        writes_files=True,
        dry_run_supported=True,
        policy_check_required=False,
        recommended_tests=("python -m pytest -p no:ddtrace --assert=plain -q tests/test_devpl_gsdlc_07_full_regression_execution_v2.py tests/test_devpl_gsdlc_07_full_regression_cli_contract.py",),
        rationale="GSDLC-07 activation enabler seals pytest collect-only nodeids and runtime session metadata under outputs/testing/full_regression; it executes no tests and never mutates source.",
    ),
    "tests.full-session.plan": DeclarativeCommandOverride(
        command_id="tests.full-session.plan",
        risk_level=CommandRiskLevel.LOW,
        side_effects=(CommandSideEffect.WRITE_FILES,),
        writes_files=True,
        dry_run_supported=True,
        policy_check_required=False,
        rationale="Builds an immutable sequential shard plan under runtime outputs only; no tests execute.",
    ),
    "tests.full-session.run": DeclarativeCommandOverride(
        command_id="tests.full-session.run",
        risk_level=CommandRiskLevel.HIGH,
        side_effects=(CommandSideEffect.EXECUTE_SUBPROCESS, CommandSideEffect.WRITE_FILES),
        writes_files=True,
        dry_run_supported=True,
        policy_check_required=False,
        rationale="Runs only sealed pytest nodeids via typed argv with shell=False; --execute is explicit, shards are sequential, runtime writes are confined to outputs/, and source fingerprint drift blocks continuation. v2.1 does not claim Approval/PolicyEngine binding.",
    ),
    "tests.full-session.resume": DeclarativeCommandOverride(
        command_id="tests.full-session.resume",
        risk_level=CommandRiskLevel.HIGH,
        side_effects=(CommandSideEffect.EXECUTE_SUBPROCESS, CommandSideEffect.WRITE_FILES),
        writes_files=True,
        dry_run_supported=True,
        policy_check_required=False,
        rationale="Resumes only UNEXECUTED nodeids in the same sealed logical session after fingerprint validation; completed nodeids are not rerun. v2.1 uses explicit --execute but does not claim Approval/PolicyEngine binding.",
    ),
    "tests.full-session.status": DeclarativeCommandOverride(
        command_id="tests.full-session.status",
        risk_level=CommandRiskLevel.LOW,
        side_effects=(CommandSideEffect.NONE,),
        writes_files=False,
        dry_run_supported=True,
        policy_check_required=False,
        rationale="Read-only aggregation of immutable full-session receipts.",
    ),
    "tests.full-session.adjudicate": DeclarativeCommandOverride(
        command_id="tests.full-session.adjudicate",
        risk_level=CommandRiskLevel.MEDIUM,
        side_effects=(CommandSideEffect.WRITE_FILES,),
        writes_files=True,
        dry_run_supported=True,
        policy_check_required=False,
        rationale="Writes final runtime adjudication only after 100% terminal accounting; incomplete sessions BLOCK and no source is modified.",
    ),
