...
_ENGLISH_ACTION_RE = re.compile(r"^(?:create|read|update|delete|manage|register|record|consult|query|identify|track|calculate|show|list|notify|export|import|approve|reject)\b", re.IGNORECASE)
...
def _item(*, item_id: str, kind: str, statement: str, source_excerpt: str, source_ref: str, confidence: str, status: str = "CANDIDATE", related_item_ids: list[str] | None = None, critical: bool = False, decision: str = "") -> dict[str, Any]:
    return {
        "id": item_id,
        "kind": kind,
...
def build_candidate_model(*, workspace_id: str, business_need: str, source_ref: str, source_sha256: str, constraints: dict[str, Any], model_policy: dict[str, Any]) -> dict[str, Any]:
    ...
    if re.search(r"empresa\s+unipersonal", need, flags=re.IGNORECASE):
        actors.append(_item(item_id="ACT-001", kind="ACTOR", statement="Persona propietaria y operadora de la empresa unipersonal", source_excerpt="empresa unipersonal", source_ref=source_ref, confidence="DERIVED_SAFE"))
    else:
        open_questions.append(_item(item_id="Q-001", kind="OPEN_QUESTION", statement="¿Quién es el actor o usuario principal del producto?", source_excerpt=need, source_ref=source_ref, confidence="AMBIGUOUS", status="OPEN", critical=True))

    outcome_match = re.search(r"\bpara\s+((?:facilitar|mantener|mejorar|reducir|aumentar|garantizar|asegurar)\b.+)$", need, flags=re.IGNORECASE)
    capability_source = need
    if outcome_match:
        outcome_text = _compact(outcome_match.group(1))
        if outcome_text:
            outcomes.append(_item(item_id="OUT-001", kind="OUTCOME", statement=outcome_text[0].upper()+outcome_text[1:], source_excerpt=outcome_match.group(0), source_ref=source_ref, confidence="EXPLICIT"))
            capability_source = need[:outcome_match.start()].rstrip(" ,.;")
    else:
        open_questions.append(_item(item_id=f"Q-{len(open_questions)+1:03d}", kind="OPEN_QUESTION", statement="¿Qué resultado de negocio observable espera obtener el Owner con el MVP?", source_excerpt=need, source_ref=source_ref, confidence="AMBIGUOUS", status="OPEN", critical=True))
    ...
    # Fallback remains conservative: retain complete clauses that visibly start with an infinitive action.
    if not actions:
    ...
    for index, action in enumerate(actions[:16], start=1):
        ...
        confidence = "AMBIGUOUS" if _capability_question(action, cap_id, "Q-000") else "EXPLICIT"
        capabilities.append(_item(item_id=cap_id, kind="CAPABILITY", statement=action, source_excerpt=action, source_ref=source_ref, confidence=confidence))
    ...
    if not capabilities:
        open_questions.append(_item(item_id=f"Q-{len(open_questions)+1:03d}", kind="OPEN_QUESTION", statement="¿Qué capacidades concretas debe ofrecer el MVP?", source_excerpt=need, source_ref=source_ref, confidence="AMBIGUOUS", status="OPEN", critical=True))

    constraints_rows = [
        _item(item_id="CON-001", kind="CONSTRAINT", statement=f"Local-first: {'sí' if bool(constraints.get('local_first', True)) else 'no'}", source_excerpt="project_constraints.local_first", source_ref=source_ref, confidence="EXPLICIT", status="CONFIRMED"),
        _item(item_id="CON-002", kind="CONSTRAINT", statement=f"Cloud obligatorio: {'sí' if bool(constraints.get('cloud_required', False)) else 'no'}", source_excerpt="project_constraints.cloud_required", source_ref=source_ref, confidence="EXPLICIT", status="CONFIRMED"),
        _item(item_id="CON-003", kind="CONSTRAINT", statement=f"Baseline de modelos: {str(model_policy.get('baseline') or 'mock-no-api')}", source_excerpt="model_policy.baseline", source_ref=source_ref, confidence="EXPLICIT", status="CONFIRMED"),
    ]
...
def normalize_owner_model(candidate: dict[str, Any], submitted: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in normalized:
        for item in submitted.get(key) or []:
            if isinstance(item, dict): incoming.append(item)
    if not incoming:
        raise ValueError("semantic model submission contains no items")

    for raw in incoming:
        kind=str(raw.get("kind") or "").upper()
        if kind not in _ALLOWED_KINDS: raise ValueError(f"invalid semantic kind: {kind}")
        item_id=str(raw.get("id") or "").strip()
        original=original_by_id.get(item_id)
        if not original:
            item_id=f"{prefixes[kind]}-{next_numbers[kind]:03d}"; next_numbers[kind]+=1
        statement=_compact(raw.get("statement") or "")
        if not statement: raise ValueError(f"empty semantic statement: {item_id}")
        status=str(raw.get("status") or "CANDIDATE").upper()
        if status not in _ALLOWED_STATUS: raise ValueError(f"invalid semantic status: {status}")
        confidence=str((original or raw).get("confidence_class") or "AMBIGUOUS").upper()
        if confidence not in _ALLOWED_CONFIDENCE: confidence="AMBIGUOUS"
        row=_item(
            item_id=item_id, kind=kind, statement=statement,
    ...
    base["quality_state"]="CONFIRMED"
    base["semantic_model_sha256"]=semantic_hash(base)
    return base


def capability_actionable(statement: str) -> bool:
    text=_compact(statement)
    if len(text) < 12: return False
    low=text.lower()
    first=low.split()[0] if low.split() else ""
    if first in _VAGUE_VERBS and not re.search(r"[,;]|\b(?:crear|consultar|actualizar|registrar|identificar|listar|calcular|mostrar|notificar|exportar|importar)\b", low):
        return False
...
def validate_confirmed_model(model: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    actors=[x for x in model.get("actors") or [] if x.get("status")=="CONFIRMED"]
    outcomes=[x for x in model.get("outcomes") or [] if x.get("status")=="CONFIRMED"]
    caps=[x for x in model.get("capabilities") or [] if x.get("status")=="CONFIRMED"]
    if not actors: findings.append({"id":"SEMANTIC_MODEL_ACTOR_REQUIRED_BLOCK","message":"At least one confirmed actor is required."})
    if not outcomes: findings.append({"id":"SEMANTIC_MODEL_OUTCOME_REQUIRED_BLOCK","message":"At least one confirmed outcome is required."})
    if not caps: findings.append({"id":"SEMANTIC_MODEL_CAPABILITY_REQUIRED_BLOCK","message":"At least one confirmed capability is required."})
    for cap in caps:
        if not capability_actionable(str(cap.get("statement") or "")):
            findings.append({"id":"SEMANTIC_MODEL_CAPABILITY_ACTION_BLOCK","message":f"Capability {cap.get('id')} is not an observable/actionable capability.","item_id":cap.get("id")})
    ...
    for question in model.get("open_questions") or []:
        if not bool(question.get("critical")): continue
        if question.get("status") != "CONFIRMED" or not str(question.get("decision") or "").strip():
            findings.append({"id":"SEMANTIC_MODEL_CRITICAL_QUESTION_BLOCK","message":f"Critical question {question.get('id')} must be resolved before generation.","item_id":question.get("id")})
    return findings


def requirement_records(model: dict[str, Any]) -> list[dict[str, Any]]:
    caps=[x for x in model.get("capabilities") or [] if x.get("status")=="CONFIRMED"]
    questions={str(x.get("id")):x for x in model.get("open_questions") or []}
    records=[]
    for index, cap in enumerate(caps, 1):
        statement=_compact(cap.get("statement") or "")
        low=statement.lower()
        fr_statement=f"El sistema debe permitir al actor autorizado {statement[0].lower()+statement[1:] if statement else statement}."
        if low.startswith(("consultar ", "listar ", "mostrar ")):
            criterion=f"Dada información existente y accesible, al ejecutar «{statement}», el sistema devuelve la información solicitada sin modificarla."
            method="TEST"
            ...
            related=[questions.get(qid) for qid in cap.get("related_item_ids") or [] if questions.get(qid)]
            decision="; ".join(str(q.get("decision") or "") for q in related if q.get("decision"))
            criterion=f"Dados datos que cumplen el criterio confirmado{': '+decision if decision else ''}, el sistema identifica los elementos correspondientes y no marca los que no lo cumplen."
            method="TEST"
        else:
            criterion=f"Mediante demostración controlada, un actor autorizado puede completar «{statement}» y observar el resultado definido por el alcance aprobado."
            method="DEMONSTRATION"
        records.append({"id":f"RF-{index:03d}","type":"FR","statement":fr_statement,"source_capability_ids":[cap.get("id")],"priority":"MUST","acceptance_criteria":[criterion],"verification_method":method,"open_decisions":[]})
    return records
...
def validate_rendered_artifact(stage_id: str, content: str, model: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if stage_id in {"product-vision","scope"}:
        for cap in [x for x in model.get("capabilities") or [] if x.get("status")=="CONFIRMED"]:
            if str(cap.get("statement") or "") not in content:
                findings.append({"id":"SEMANTIC_RENDER_CAPABILITY_TRACE_BLOCK","message":f"Rendered {stage_id} omitted confirmed capability {cap.get('id')}."})
