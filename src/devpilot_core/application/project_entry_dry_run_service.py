...
class ProjectEntryDryRunApplicationService:
    ...
    def dry_run(self, *, intake: Mapping[str, Any], timeout_seconds: float = 3.0) -> CommandResult:
        return ProjectEntryDryRunService(self.root, timeout_seconds=timeout_seconds).dry_run(intake=intake)

    def revalidate(self, *, intake: Mapping[str, Any], expected_plan_hash: str, expected_preimage_hash: str, timeout_seconds: float = 3.0) -> CommandResult:
