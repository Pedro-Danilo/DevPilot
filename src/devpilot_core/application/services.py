...
class ApplicationService:
    ...
    def __init__(self, root: Path, *, enforce_workspace_paths: bool = False, approval_auth_store: LocalAuthStore | None = None) -> None:
        ...
        self.reconciliation = ReconciliationApplicationService(self.root, context_resolver=self.ui_workspace_context)
    ...
    @property
    def release_package(self) -> ReleasePackageJobApplicationService:
        """Lazily construct the GSDLC-11-B typed local release package workbench."""
    ...
    @property
    def release_metadata(self) -> ReleaseMetadataApplicationService:
        """Lazily construct the GSDLC-11-D version/notes/approval/tag workbench."""
    ...
    def guided_pre_code_save_draft(self, *, stage_id: str, content: str, mode: str, actor: str, actor_role: str, session_principal: str, effective_roles: list[str], workspace_scopes: list[str], semantic_model: dict[str, Any] | None = None) -> CommandResult:
        return self.pre_code_wizard.save_draft(stage_id=stage_id, content=content, mode=mode, actor=actor, actor_role=actor_role, session_principal=session_principal, effective_roles=effective_roles, workspace_scopes=workspace_scopes, semantic_model=semantic_model)
    ...
    def settings_agent_execution_create_authenticated(self, *, payload: dict[str, Any], principal: AuthenticatedPrincipal, session: SessionContext) -> CommandResult:
        ...
        if not ({"owner", "developer"} & roles):
            return CommandResult("agent execution session create", False, ExitCode.BLOCK, "Bounded agent execution requires owner or developer human role.", {"summary": {"created": False}}, [Finding("AGENT_EXECUTION_RBAC_ROLE_DENY", "Human role is not authorized to create bounded agent session.", Severity.BLOCK)])
        return self.agent_execution.create_session(role_id=str(payload.get("role_id") or "requirements"), step_id=str(payload.get("step_id") or "requirements"), actor_id=principal.actor_id, mode=str(payload.get("mode") or "fake-local"))

    def settings_agent_execution_intent_authenticated(self, *, session_id: str, payload: dict[str, Any], principal: AuthenticatedPrincipal, session: SessionContext) -> CommandResult:
        del session
        roles = self.rbac.enforcer.canonical_roles(principal)
        role = "owner" if "owner" in roles else (roles[0] if roles else "unknown")
        return self.agent_execution.tool_intent(session_id=session_id, payload=payload, actor_id=principal.actor_id, role_at_decision=role)

    def settings_agent_execution_handoff_authenticated(self, *, session_id: str, payload: dict[str, Any], principal: AuthenticatedPrincipal, session: SessionContext) -> CommandResult:
        del session
        return self.agent_execution.handoff(session_id=session_id, to_role_id=str(payload.get("to_role_id") or "review"), to_step_id=str(payload.get("to_step_id") or "validation"), reason=str(payload.get("reason") or "Explicit bounded handoff"), human_checkpoint=bool(payload.get("human_checkpoint", False)), actor_id=principal.actor_id)

    def settings_agent_execution_cancel_authenticated(self, *, session_id: str, reason: str, principal: AuthenticatedPrincipal, session: SessionContext, kill: bool = False) -> CommandResult:
        del session
        roles = set(self.rbac.enforcer.canonical_roles(principal))
        if kill and "owner" not in roles:
            return CommandResult("agent execution kill", False, ExitCode.BLOCK, "Kill switch requires owner human role.", {}, [Finding("AGENT_EXECUTION_KILL_RBAC_DENY", "Only owner may activate kill switch.", Severity.BLOCK)])
        if not ({"owner", "developer"} & roles):
            return CommandResult("agent execution cancel", False, ExitCode.BLOCK, "Cancel requires owner or developer human role.", {}, [Finding("AGENT_EXECUTION_CANCEL_RBAC_DENY", "Role is not authorized to cancel session.", Severity.BLOCK)])
        return self.agent_execution.cancel(session_id=session_id, actor_id=principal.actor_id, reason=reason, kill=kill)
    ...
    def release_package_status(self, *, actor: str, actor_roles: list[str], workspace_scopes: list[str]) -> CommandResult:
        return self.release_package.status(actor=actor, actor_roles=actor_roles, workspace_scopes=workspace_scopes)

    def release_package_plan(self, *, actor: str, actor_roles: list[str], workspace_scopes: list[str]) -> CommandResult:
        return self.release_package.plan(actor=actor, actor_roles=actor_roles, workspace_scopes=workspace_scopes)
    ...
    def release_metadata_status(self, *, actor: str, actor_roles: list[str], workspace_scopes: list[str]) -> CommandResult:
        return self.release_metadata.status(actor=actor, actor_roles=actor_roles, workspace_scopes=workspace_scopes)

    def release_metadata_prepare(self, *, actor: str, actor_roles: list[str], workspace_scopes: list[str], mode: str, version: str, manual_notes: str = "", agent_proposal: str = "") -> CommandResult:
        return self.release_metadata.prepare(actor=actor, actor_roles=actor_roles, workspace_scopes=workspace_scopes, mode=mode, version=version, manual_notes=manual_notes, agent_proposal=agent_proposal)

    def release_metadata_tag_plan(self, *, actor: str, actor_roles: list[str], workspace_scopes: list[str]) -> CommandResult:
        return self.release_metadata.tag_plan(actor=actor, actor_roles=actor_roles, workspace_scopes=workspace_scopes)

    def release_metadata_approve(self, *, actor: str, actor_roles: list[str], workspace_scopes: list[str], plan_id: str, plan_hash: str, reason: str, ttl_minutes: int = 30) -> CommandResult:
        return self.release_metadata.approve(actor=actor, actor_roles=actor_roles, workspace_scopes=workspace_scopes, plan_id=plan_id, plan_hash=plan_hash, reason=reason, ttl_minutes=ttl_minutes)
    ...
    def reconciliation_status(self, *, actor: str, roles: list[str], workspace_scopes: list[str], session_created_at: str, rotation_counter: int) -> CommandResult:
        return self.reconciliation.status(actor=actor, roles=roles, workspace_scopes=workspace_scopes, session_created_at=session_created_at, rotation_counter=rotation_counter)

    def reconciliation_baseline(self, *, actor: str, roles: list[str], workspace_scopes: list[str], session_created_at: str, rotation_counter: int, execute: bool, confirmation: str) -> CommandResult:
        return self.reconciliation.baseline(actor=actor, roles=roles, workspace_scopes=workspace_scopes, session_created_at=session_created_at, rotation_counter=rotation_counter, execute=execute, confirmation=confirmation)

    def reconciliation_adopt(self, *, actor: str, roles: list[str], workspace_scopes: list[str], session_created_at: str, rotation_counter: int, execute: bool, confirmation: str) -> CommandResult:
        return self.reconciliation.adopt(actor=actor, roles=roles, workspace_scopes=workspace_scopes, session_created_at=session_created_at, rotation_counter=rotation_counter, execute=execute, confirmation=confirmation)
...
def _operation_dispatch(service: ApplicationService) -> dict[str, OperationHandler]:
    return {
        ...
        "guided_sdlc.pre_code.draft": lambda payload: service.guided_pre_code_save_draft(stage_id=str(payload.get("stage_id", "")), content=str(payload.get("content", "")), mode=str(payload.get("mode", "MANUAL")), actor=str(payload.get("actor", "")), actor_role=str(payload.get("actor_role", "")), session_principal=str(payload.get("session_principal", "")), effective_roles=list(payload.get("effective_roles") or []), workspace_scopes=list(payload.get("workspace_scopes") or []), semantic_model=(dict(payload.get("semantic_model") or {}) if payload.get("semantic_model") is not None else None)),
        ...
        "release.metadata.status": lambda payload: service.release_metadata_status(actor=str(payload.get("actor", "")), actor_roles=list(payload.get("actor_roles") or []), workspace_scopes=list(payload.get("workspace_scopes") or [])),
        "release.metadata.prepare": lambda payload: service.release_metadata_prepare(actor=str(payload.get("actor", "")), actor_roles=list(payload.get("actor_roles") or []), workspace_scopes=list(payload.get("workspace_scopes") or []), mode=str(payload.get("mode", "MANUAL")), version=str(payload.get("version", "")), manual_notes=str(payload.get("manual_notes", "")), agent_proposal=str(payload.get("agent_proposal", ""))),
        ...
        "reconciliation.status": lambda payload: service.reconciliation_status(actor=str(payload.get("actor", "")), roles=list(payload.get("roles") or []), workspace_scopes=list(payload.get("workspace_scopes") or []), session_created_at=str(payload.get("session_created_at", "")), rotation_counter=int(payload.get("rotation_counter", 0))),
        "reconciliation.baseline": lambda payload: service.reconciliation_baseline(actor=str(payload.get("actor", "")), roles=list(payload.get("roles") or []), workspace_scopes=list(payload.get("workspace_scopes") or []), session_created_at=str(payload.get("session_created_at", "")), rotation_counter=int(payload.get("rotation_counter", 0)), execute=bool(payload.get("execute", False)), confirmation=str(payload.get("confirmation", ""))),
        "reconciliation.adopt": lambda payload: service.reconciliation_adopt(actor=str(payload.get("actor", "")), roles=list(payload.get("roles") or []), workspace_scopes=list(payload.get("workspace_scopes") or []), session_created_at=str(payload.get("session_created_at", "")), rotation_counter=int(payload.get("rotation_counter", 0)), execute=bool(payload.get("execute", False)), confirmation=str(payload.get("confirmation", ""))),
...
def _capabilities() -> list[ServiceCapability]:
    rows = [
        ...
        ("guided_sdlc.pre_code.status", "Read resumable governed pre-code wizard state and stage advisor, including C-01 deterministic local derivation.", "none", True, "GET /api/v1/guided-sdlc/pre-code"),
        ("guided_sdlc.pre_code.draft", "Generate a draft-first C-01 DEVPL_MOCK proposal from the internal Semantic Model, persist Owner edits/decision updates in runtime state, and keep managed source unchanged until approved apply.", "runtime_draft_only", False, "POST /api/v1/guided-sdlc/pre-code/stages/{stage_id}/draft"),
...
def _routes() -> list[InterfaceRouteContract]:
    route_specs = [
        ...
        ("APP-ROUTE-GSDLC-05-E-PRE-CODE-DRAFT", "POST", "/api/v1/guided-sdlc/pre-code/stages/{stage_id}/draft", "guided_sdlc.pre_code.draft", ["GSDLC-13-C draft-first MANUAL/IMPORT/DEVPL_MOCK runtime DRAFT and governed Decision Inbox updates only; no managed source write."]),
        ...
        ("APP-ROUTE-136", "GET", "/api/v1/settings/agent-execution", "settings.agent_execution", ["GSDLC-07-D SkillToolPolicyView projection; ModelRouteDecision grants no tool authority."]),
        ("APP-ROUTE-137", "POST", "/api/v1/settings/agent-execution/sessions", "settings.agent_execution.create", ["GSDLC-07-D human-session bounded fake-local agent execution session."]),
        ("APP-ROUTE-138", "POST", "/api/v1/settings/agent-execution/sessions/{session_id}/tool-intents", "settings.agent_execution.intent", ["GSDLC-07-D ToolIntent -> deterministic ToolExecutionDecision; forbidden tools remain non-executable."]),
