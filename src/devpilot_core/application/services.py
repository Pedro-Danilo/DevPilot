...
def _capabilities() -> list[ServiceCapability]:
    rows = [
        ...
        ("guided_sdlc.pre_code.draft", "Prepare/confirm the C-01 Semantic Model when required and persist one current-stage MANUAL/IMPORT/DEVPL_MOCK DRAFT in platform runtime state without managed source write.", "runtime_draft_only", False, "POST /api/v1/guided-sdlc/pre-code/stages/{stage_id}/draft"),
...
def _routes() -> list[InterfaceRouteContract]:
    route_specs = [
        ...
        ("APP-ROUTE-GSDLC-05-E-PRE-CODE-DRAFT", "POST", "/api/v1/guided-sdlc/pre-code/stages/{stage_id}/draft", "guided_sdlc.pre_code.draft", ["GSDLC-13-C current-stage MANUAL/IMPORT/DEVPL_MOCK runtime DRAFT only; no managed source write."]),
