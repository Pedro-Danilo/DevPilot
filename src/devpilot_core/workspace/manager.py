...
paths:
  ...
  interface_contract: "docs/07_interfaces/internal_application_contract.md"
runtime:
...
def parse_project_yaml_metadata(path: Path) -> dict[str, Any]:
    """Parse the bounded DevPilot project.yaml metadata contract.

    This remains dependency-free and is intentionally *not* a general YAML
    parser. It supports the one-level mappings written by DevPilot bootstrap,
    including model policy, project constraints and deferred stack metadata.
    Unknown sections are ignored fail-closed by consumers.
    """
    ...
    standards: list[str] = []
    mapping_sections = {"model_policy", "project_constraints", "stack"}
    for raw_line in path.read_text(encoding="utf-8").splitlines():
        ...
        if not line.startswith(" ") and line.endswith(":"):
            ...
            if current_section in mapping_sections:
                metadata.setdefault(current_section, {})
            continue
        ...
        elif current_section in mapping_sections and line.startswith("  ") and ":" in line:
            key, value = line.split(":", 1)
            section = metadata.setdefault(current_section, {})
