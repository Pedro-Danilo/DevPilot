...
class DashboardBuildResult:
    ...
    and validates the JSON against ``MaturityDashboard``. POST-H-002-D exposes
    this result through ApplicationService/CLI and persists reports only when
    the explicit CLI adapter receives ``--write-report``.
    """
...
class MaturityDashboardBuilder:
    ...
    It does not write ``outputs/reports`` itself; POST-H-002-D keeps report
    persistence in the ApplicationService/CLI adapter so the core builder stays
...
def render_maturity_dashboard_markdown(dashboard: MaturityDashboard, *, source_paths: Sequence[str] = ()) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "This report is generated by the POST-H-002 maturity dashboard builder and exposed through the POST-H-002-D CLI/ApplicationService boundary.",
            "It does not enable remote execution, connector write, plugin execution, external APIs or source mutations.",
            "Persistent report writing is explicit and limited to outputs/reports/maturity_dashboard.json and outputs/reports/maturity_dashboard.md when the CLI uses --write-report.",
