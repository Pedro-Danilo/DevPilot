...
def test_sprint_81_release_verification_artifacts_exist_and_are_synchronized() -> None:
    ...
    assert "FUNC-SPRINT-81 — Checksums, smoke tests y verificación de release" in readme
    assert "FUNC-SPRINT-81 — Operación de checksums" in runbook
    assert 'source_repo: "repo_DevPilot_Local_103.zip"' in backlog_g
    assert 'last_completed_sprint: "FUNC-SPRINT-81"' in backlog_g
    assert 'next_sprint: "FUNC-SPRINT-82"' in backlog_g
