...
def test_sprint_94_readme_runbook_and_backlog_are_synchronized() -> None:
    ...
    assert 'source_repo: "repo_DevPilot_Local_122.zip"' in backlog
    assert 'last_completed_sprint: "FUNC-SPRINT-94"' in backlog
    assert 'next_sprint: "FUNC-SPRINT-95"' in backlog
    assert 'first_open_sprint: "FUNC-SPRINT-95"' in backlog
...
def test_sprint_94_release_changelog_mentions_range() -> None:
    ...
    assert "Range: `FUNC-SPRINT-74` → `FUNC-SPRINT-94`" in changelog
