...
def test_capability_parity_summary_preserves_uoc011_historical_floor_and_tracks_current_entries():
 r=j('.devpilot/interfaces/ui_capability_registry.json'); frozen=j('.devpilot/interfaces/governed_job_capability_registry.json'); actual=Counter(str(x['parity_status']) for x in r['capabilities']); historical_total=int(frozen['summary']['uoc_011_capabilities_total_at_close']); current_total=len(r['capabilities'])
 assert historical_total==193; assert current_total>=historical_total; assert r['summary']['cli_commands_total']==current_total; assert r['summary']['cli_commands_classified_total']==current_total; assert r['summary']['parity_status_counts']==dict(sorted(actual.items())); assert r['summary']['current_ui_native_or_read_only_total']==actual['UI-NATIVE']+actual['UI-READ-ONLY']; assert r['summary']['uoc_007_governed_job_capabilities_total']==current_total; assert r['summary']['uoc_007_governed_job_capabilities_total_at_close']==historical_total; assert r['summary']['uoc_011_authorized'] is True
...
def test_final_reconciliation_lifecycle_is_candidate_or_closed_without_regressing_pilot():
 s=j('.devpilot/project_state.json'); assert s['post_h_eval_002_current_micro_sprint']=='POST-H-EVAL-002-02-B' and s['post_h_eval_002_next_micro_sprint']=='POST-H-EVAL-002-02-C'; status=s['ui_operational_console_final_reconciliation_status']; assert status in {'implemented-initial/pending-authoritative-windows-final-closure','closed/PASS'}
 ...
 else: assert s['ui_operational_console_authoritative_baseline']==REPO340 and s['ui_operational_console_program_status']=='CLOSED/PASS' and s['ui_operational_console_final_reconciliation_source_commit']; assert s['current_repo'].startswith('repo_DevPilot_Local_') and int(s['current_repo'].split('_')[3]) >= 340; assert s['ui_operational_console_final_browser_runtime_matrix_status']=='PASS/108-of-108'; assert s['ui_operational_console_final_full_regression_status']=='PASS'; assert s['ui_operational_console_program_administrative_closure'] is True
