...
def workspace(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    store=StoryExecutionStore(root,workspace_id='story-closure-fixture')
    store.save_state(StoryExecutionState(execution_id='story-exec-09e1234567890abcdef1234',workspace_id='story-closure-fixture',project_id='story-closure-fixture',story_id='STORY-09E',story_version='1.0.0',status=StoryExecutionStatus.IN_PROGRESS,sequence=1,dor_report_sha256='a'*64,context_pack_id='story-context-09e1234567890abcdef12',context_pack_sha256='b'*64,created_at_utc='2026-09-08T18:00:00+00:00',updated_at_utc='2026-09-08T18:00:00+00:00'))
    return root
...
def test_01_approved_atomic_apply_advances_active_story_to_changes_ready(workspace:Path,runtime)->None:
    app,client=runtime; p=plan(app,'def answer():\n    return 43\n'); r=app.story_source_change_apply(plan_id=p['plan_id'],plan_hash=p['plan_hash'],approval_id=approve(app,client,p),actor='local-owner',actor_role='owner'); assert r.ok,r.to_dict(); assert r.data['story_execution_state']['status']=='CHANGES_READY'; assert r.data['story_state_transition']['trigger']=='approved-atomic-source-apply'; state=StoryExecutionStore(workspace,workspace_id='story-closure-fixture').load_state(); assert state and state.status is StoryExecutionStatus.CHANGES_READY and state.sequence==2

def test_02_wrong_role_or_blocked_apply_does_not_advance_story(workspace:Path,runtime)->None:
    app,_=runtime; p=plan(app,'def answer():\n    return 44\n'); r=app.story_source_change_apply(plan_id=p['plan_id'],plan_hash=p['plan_hash'],approval_id='missing',actor='developer',actor_role='developer'); assert not r.ok; state=StoryExecutionStore(workspace,workspace_id='story-closure-fixture').load_state(); assert state and state.status is StoryExecutionStatus.IN_PROGRESS

def test_03_rollback_restores_source_and_story_state_remains_monotonic(workspace:Path,runtime)->None:
    app,client=runtime; before=(workspace/'src/app.py').read_bytes(); p=plan(app,'def answer():\n    return 45\n'); applied=app.story_source_change_apply(plan_id=p['plan_id'],plan_hash=p['plan_hash'],approval_id=approve(app,client,p),actor='local-owner',actor_role='owner'); ex=applied.data['execution']; rr=app.story_source_change_rollback_approval_request(execution_id=ex['execution_id'],actor='local-owner',actor_role='owner',reason='09-E rollback verification'); assert rr.ok; aid=rr.data['approval']['approval_id']; d=client.post(f'/api/v1/approvals/{aid}/approve',json={'reason':'Owner approves rollback'},headers=csrf(client)); assert d.status_code==200; rb=app.story_source_change_rollback(execution_id=ex['execution_id'],approval_id=aid,actor='local-owner',actor_role='owner'); assert rb.ok and rb.data['source_hash_parity'] is True and (workspace/'src/app.py').read_bytes()==before; state=StoryExecutionStore(workspace,workspace_id='story-closure-fixture').load_state(); assert state and state.status is StoryExecutionStatus.CHANGES_READY
