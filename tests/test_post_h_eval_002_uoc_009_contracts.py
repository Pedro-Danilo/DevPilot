...
def test_uoc009_routes_and_typed_execution_subset_without_no_go_relaxation() -> None:
    ...
    enabled=[x for x in jobs['capabilities'] if x['runtime']['execution_enabled']]; assert len(enabled)==10; assert all(x['runtime']['adapter_id']=='uoc009.quality.typed-worker' for x in enabled); assert jobs['safety']['arbitrary_shell_allowed'] is False
