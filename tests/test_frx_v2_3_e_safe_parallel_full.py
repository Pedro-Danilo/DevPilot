...
def test_e_plan_never_parallelizes_unclassified_nodes(tmp_path: Path):
 p=pristine_planner(tmp_path).build(collection(),environment_fingerprint='windows-pytest-devpilot-gsdlc07e-v1')
...
def test_e_serial_lane_is_coarsened_manifest_based(tmp_path: Path):
 p=pristine_planner(tmp_path).build(collection(),environment_fingerprint='windows-pytest-devpilot-gsdlc07e-v1')
 assert p['serial_shards_total'] < max(1,(p['serial_total']+49)//50) and max(len(s['nodeids']) for s in p['serial_shards'])<=200
def test_e_performance_attribution_separates_three_metrics_and_available_not_default(tmp_path: Path):
 p=pristine_planner(tmp_path).build(collection(),environment_fingerprint='windows-pytest-devpilot-gsdlc07e-v1')
...
def test_e_safety_block_on_second_full_or_incomplete_accounting(tmp_path: Path):
 p=pristine_planner(tmp_path).build(collection(),environment_fingerprint='windows-pytest-devpilot-gsdlc07e-v1')
