...
def test_project_global_state_schema_and_docs_are_synchronized() -> None:
    ...
    # current_repo is mutable global state; freeze the naming contract, not every future repo number.
    current_repo = str(state.get("current_repo", ""))
    parts = current_repo.split("_", 4)
    assert len(parts) == 5 and parts[:3] == ["repo", "DevPilot", "Local"]
    assert parts[3].isdigit() and int(parts[3]) >= 319
    assert current_repo.endswith(".zip")
