...
def test_miasi_semantic_validator_current_bundle_passes_with_read_only_report() -> None:
    ...
    assert result.data["summary"]["rules_total"] >= 13
...
def test_miasi_semantic_validator_includes_post_h_004_d_rules() -> None:
    ...
    assert "SEM-OBSERVABILITY-001" in rule_ids
    assert "SEM-EVAL-COVERAGE-001" in rule_ids
    assert "SEM-TEST-CONTRACT-COVERAGE-001" in rule_ids
    assert result.data["tests_executed"] is False
