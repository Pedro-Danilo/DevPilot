...
def test_post_h_030_b_governance_artifacts_are_synchronized() -> None:
    ...
    assert state["current_repo"] == "repo_DevPilot_Local_285_POST_H_030_B.zip"
    assert state["post_h_030_status"] == "active/implemented-initial-post-h-030-b"
    ...
    assert 'current_micro_sprint: "POST-H-030-B"' in backlog
    assert 'next_micro_sprint: "POST-H-030-C"' in backlog
