...
def test_operator_health_contracts_and_route_registry_are_synchronized() -> None:
    ...
    assert route_summary["routes_total"] == 36
    assert route_summary["fastapi_route_keys_total"] == 36
    assert route_summary["canonical_router_route_keys_total"] == 36
    assert route_summary["application_service_routes_total"] == 32
