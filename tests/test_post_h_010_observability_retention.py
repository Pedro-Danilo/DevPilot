...
def test_post_h_010_d_redacted_export_contracts_are_registered() -> None:
    tcr = read_json(".devpilot/testing/test_contract_registry.json")
    tcr_v2 = read_json(".devpilot/testing/test_contract_registry_v2.json")

    contract = next(item for item in tcr["contracts"] if item["contract_id"] == "post-h-010-observability-redacted-export")
    assert contract["owner"] == "POST-H-010-D"
    assert "tests/test_observability_export.py" in contract["test_files"]
    assert "src/devpilot_core/observability/export.py" in contract["validates"]
    assert "docs/schemas/observability_redacted_export.schema.json" in contract["validates"]
    assert contract["mutable_global_state_allowed"] is False

    contract_v2 = next(item for item in tcr_v2["contracts"] if item["contract_id"] == "post-h-010-observability-redacted-export")
