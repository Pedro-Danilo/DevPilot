...
def test_application_service_boundary_report_builder_is_read_only_and_finds_bypasses() -> None:
    ...
    assert summary["api_bound_or_exempt_total"] == summary["api_routes_total"]
    assert summary["api_unbound_total"] == 0
    api_exempt_routes = [
        route
        for route in report["interfaces"]["api"]["routes"]
        if route.get("application_service_boundary_exempt") is True
    ]
    exempt_paths = {route["path"] for route in api_exempt_routes}
    assert "/api/v1/security/posture" in exempt_paths
    assert {
    ...
    assert all(route["boundary_exemption_reason"] for route in api_exempt_routes)
