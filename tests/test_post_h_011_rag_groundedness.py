...
def test_post_h_011_backlog_is_approved_and_schema_fixtures_are_documented() -> None:
    ...
    assert 'current_micro_sprint: "POST-H-011-A"' in backlog
    assert 'next_micro_sprint: "POST-H-011-B"' in backlog
...
def test_post_h_011_a_project_state_notes_are_synchronized() -> None:
    ...
    assert state["current_repo"] == "repo_DevPilot_Local_187_POST_H_011_A.zip"
    assert any("POST-H-011-A starts RAG groundedness evals" in note for note in state["notes"])
    assert "Último micro-sprint implementado: `POST-H-011-A" in readme
    assert "POST-H-011-A — Schema y fixtures de groundedness" in runbook
