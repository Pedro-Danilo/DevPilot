...
def test_evidence_freshness_scanner_reads_current_repo_without_writing_outputs() -> None:
    ...
    assert result.data["report"]["repo_version"] == "repo_DevPilot_Local_270_POST_H_027_B.zip"
        ...
        [
            {
                ...
                "expected_fields": {"current_repo": "repo_DevPilot_Local_270_POST_H_027_B.zip"},
            },
            {
...
def test_evidence_freshness_optional_runtime_absence_is_not_applicable(tmp_path: Path) -> None:
    _write_minimal_workspace(tmp_path, current_repo="repo_DevPilot_Local_270_POST_H_027_B.zip")
    criteria = _criteria(
        [
            {
...
def _report_payload(*, decision: str) -> dict:
    return {
        ...
        "decision": decision,
        "repo_version": "repo_DevPilot_Local_270_POST_H_027_B.zip",
        "criteria_id": "test",
...
def _criteria(evidence: list[dict]) -> dict:
    return {
        ...
        "expected_current_repo": "repo_DevPilot_Local_270_POST_H_027_B.zip",
        "scope": "local-release-candidate",
        "no_go_gates": {
