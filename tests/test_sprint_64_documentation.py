...
def test_sprint_64_artifacts_exist_and_are_synchronized() -> None:
    ...
    assert "Último hito: `FUNC-SPRINT-64" in readme
    assert "Siguiente hito: `FUNC-SPRINT-65" in readme
    ...
    assert 'source_repo: "repo_DevPilot_Local_78.zip"' in backlog
    assert 'first_open_sprint: "FUNC-SPRINT-65"' in backlog
    assert 'last_completed_sprint: "FUNC-SPRINT-64"' in backlog
    assert 'next_sprint: "FUNC-SPRINT-65"' in backlog
    assert "## Estado de implementación Sprint 64" in backlog
    assert 'next_sprint: "FUNC-SPRINT-65"' in functional_backlog
...
def test_sprint_64_scope_does_not_implement_server_frontend_or_desktop() -> None:
    ...
    assert not (ROOT / "src" / "devpilot_core" / "interfaces" / "api").exists()
