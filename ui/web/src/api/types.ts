...
export interface PreCodeSemanticItem {
  ...
  critical?: boolean;
  decision?: string;
  required_stage?: 'product-vision' | 'scope' | 'requirements' | string;
}
...
export interface PreCodeSemanticModel {
  ...
  quality_state: 'REVIEW_REQUIRED' | 'DRAFT_READY' | 'CONFIRMED' | string;
...
// GSDLC-12-B branch/external-edit reconciliation contracts.
export interface WorkspaceDriftChange { kind:'MODIFY'|'ADD'|'DELETE'|'RENAME'|'UNTRACKED'|string; status?:string; path:string; old_path?:string|null; baseline_hash?:string|null; current_hash?:string|null; linked_to_engineering_authority?:boolean; }
export interface WorkspaceDriftSnapshot { schema_id?:string; schema_version?:string; workspace_id?:string; baseline_present?:boolean; head_relation?:string; branch_switched?:boolean; engineering_state_changed?:boolean; worktree_dirty?:boolean; current?:{head?:string;tree?:string;branch?:string;detached?:boolean}; baseline?:Record<string,unknown>; changes?:WorkspaceDriftChange[]; change_counts?:Record<string,number>; authority_linked_change_count?:number; snapshot_sha256?:string; }
export interface ReconciliationReport { classification:'NO_CONFLICT'|'REVALIDATE'|'REPLAN_REQUIRED'|'MANUAL_RECONCILIATION_REQUIRED'|'READ_ONLY_BLOCK'|string; reason_codes?:string[]; risk?:string; recommended_next_action?:string; snapshot?:WorkspaceDriftSnapshot; authority_invalidations?:Array<{authority:string;state:string;reason_code?:string;auto_reuse?:boolean}>; draft_recovery?:{draft_refs?:string[];drafts_preserved?:boolean;discard_performed?:boolean;copy_or_export_before_discard?:boolean}; pending_recovery_work?:Array<Record<string,unknown>>; safe_recovery_plan?:string[]; forbidden_automatic_git_operations?:string[]; safety?:Record<string,unknown>; report_sha256?:string; }
export interface ReconciliationStatusData { reconciliation:ReconciliationReport; roles?:string[]; safety?:Record<string,unknown>; }
