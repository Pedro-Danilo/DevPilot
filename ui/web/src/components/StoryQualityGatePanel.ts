import { DevPilotApiClient, clearQualityRemediationHandoff, readQualityRemediationHandoff, readQualityStoryContext, writeQualityRemediationHandoff, writeQualityStoryContext } from '../api/client';
...
export function renderStoryQualityGatePanel(tokenProvider:()=>string): HTMLElement {
  ...
  async function remediate(finding:StoryQualityFinding,mode:'manual'|'agent'):Promise<void>{
    if(!report)return;busy=true;draw();try{
      let resolvedSourceId=agentSourceId.trim();
      if(mode==='agent'&&!resolvedSourceId){
        const [planResponse,sourcesResponse]=await Promise.all([client().storyTestPlan(report.story_test_plan_id),client().storyCodeSources()]);
        const plan=((planResponse.data as any)?.story_test_plan??{}) as Record<string,any>;
        const changedPaths=(plan.changed_paths??[]) as string[];
        const sources=((sourcesResponse.data as any)?.sources??[]) as Array<Record<string,any>>;
        if(changedPaths.length!==1)throw new Error('La remediación agentic E2E requiere exactamente un changed path o un Source ID explícito.');
        const match=sources.find(row=>String(row.relative_path??'')===String(changedPaths[0]));
        resolvedSourceId=String(match?.source_id??'');
        if(!resolvedSourceId)throw new Error('No se pudo resolver server-side el Source ID del único changed path.');
        agentSourceId=resolvedSourceId;
      }
      const r=await client().storyQualityPlanRemediation(report.report_id,{report_hash:report.report_hash,finding_id:finding.finding_id,mode,instruction:mode==='agent'?`Propón una corrección bounded para ${finding.finding_id}. No apliques cambios.`:'',source_id:mode==='agent'?resolvedSourceId:undefined,agent_mode:'mock'});trace=(r.data as any).remediation_trace as StoryQualityRemediationTrace;writeQualityRemediationHandoff({source_report_id:report.report_id,remediation_trace_id:trace.trace_id,story_execution_id:report.story_execution_id});setNotice(mode==='manual'?'PASS · handoff de remediación manual preparado; source sigue gobernado por GSDLC-09.':'PASS · propuesta agentic proposal-only preparada; Source ID resuelto desde el TestPlan current; ToolIntent no concedió autoridad.','pass');
    }catch(e){setNotice(errorText(e),'block');}busy=false;draw();
  }

  async function recoverRemediation():Promise<void>{
    const handoff=readQualityRemediationHandoff();context=readQualityStoryContext();
    if(!handoff||!context){setNotice('BLOCK · no hay handoff de remediación y StoryTestPlan successor recuperables en esta sesión.','block');return;}
    busy=true;draw();try{const r=await client().storyQualityRemediations(handoff.source_report_id);const rows=((r.data as any).remediation_traces??[]) as StoryQualityRemediationTrace[];const found=rows.find(row=>row.trace_id===handoff.remediation_trace_id);if(!found)throw new Error('RemediationTrace current no coincide con el handoff UX.');trace=found;successorId=context.test_plan_id;successorHash=context.test_plan_hash;setNotice('PASS · remediación recuperada desde autoridad server-side; StoryTestPlan successor cargado automáticamente.','pass');}catch(e){setNotice(errorText(e),'block');}busy=false;draw();
  }
  ...
  function useSuccessor():void{
    ...
    writeQualityStoryContext({test_plan_id:successorId.trim(),test_plan_hash:successorHash.trim()}); context=readQualityStoryContext(); report=null; trace=null; clearQualityRemediationHandoff(); setNotice('PASS · StoryTestPlan successor ahora es el contexto Quality. Evalúa nuevamente para una decisión fresca.','pass'); draw();
  }
  ...
  function draw():void{
    ...
    const ctx=document.createElement('section');ctx.className='story-quality-context';ctx.innerHTML='<h4>Contexto StoryTestPlan</h4>';const cp=document.createElement('p');cp.dataset.qualityContext='true';cp.textContent=context?`${context.test_plan_id} · ${context.test_plan_hash.slice(0,12)}…`:'Sin contexto. Planifica StoryValidationJobs desde Story Code Workbench.';ctx.append(cp);const evalBtn=button('Evaluar Quality Gate');evalBtn.dataset.evaluateStoryQuality='true';evalBtn.disabled=busy||!context;evalBtn.addEventListener('click',()=>void evaluate());ctx.append(evalBtn);const recoverBtn=button('Recuperar remediación E2E');recoverBtn.className='button-secondary';recoverBtn.dataset.recoverE2eRemediation='true';recoverBtn.disabled=busy||!readQualityRemediationHandoff();recoverBtn.addEventListener('click',()=>void recoverRemediation());ctx.append(recoverBtn);host.append(ctx);
