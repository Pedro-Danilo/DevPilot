...
export function renderSessionBanner(session: AuthSessionContext, onLogout: () => void): HTMLElement {
  ...
  const roles=document.createElement('div'); roles.className='session-banner__roles expert-only'; roles.textContent=`Roles: ${session.principal.roles.join(', ')}`;
  const account=document.createElement('a'); account.href='/account'; account.textContent='Cuenta';
