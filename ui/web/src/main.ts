...
import { renderReportTraceView } from './pages/ReportTraceView';
...
const UI_ROUTES: UiRoute[] = [
  ...
  { path: '/reports', routeId: 'ui.reports', title: 'Reports' },
  { path: '/traces', routeId: 'ui.traces', title: 'Traces' },
  { path: '/approvals', routeId: 'ui.approvals', title: 'Approval Center' },
  { path: '/settings', routeId: 'ui.settings', title: 'Settings' },
];
...
function renderApplication(target: HTMLElement): void {
  ...
  if (!route) {
    page.append(renderNotFound(currentPath));
  } else if (route.path === '/') {
    renderDashboard(page);
  } else {
    page.append(renderRouteHeader(route));
    if (route.path === '/reports' || route.path === '/traces') {
      page.append(renderReportTraceView(() => readStoredToken()));
    } else if (route.path === '/approvals') {
      page.append(renderApprovalCenterView(() => readStoredToken()));
    } else if (route.path === '/settings') {
      const wrapper = document.createElement('div');
      wrapper.innerHTML = renderSettingsView(
        new DevPilotApiClient({ token: readStoredToken() }),
        () => readStoredToken()
      );
      page.append(wrapper);
    }
