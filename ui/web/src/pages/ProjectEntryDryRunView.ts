...
export function renderProjectEntryDryRunView(options: ProjectEntryViewOptions): HTMLElement {
  ...
  const businessNeed=textareaField('Necesidad de negocio / problema a resolver *','business-need','Describe qué necesita resolver el usuario o negocio. No diseñes todavía la solución ni el stack.'); businessNeed.wrapper.classList.add('project-entry-create-only');
  const modelPolicy=selectField('Model policy del proyecto','model-policy',[['mock-no-api','Mock / sin API externa · baseline del piloto']]); modelPolicy.wrapper.classList.add('project-entry-create-only');
  const constraintsConfirm=checkboxField('Confirmo constraints del piloto','project-constraints-confirm','Local-first; cloud no obligatorio; operator_project_writes=0; shell arbitrario, red silenciosa y remote Git execute deshabilitados.'); constraintsConfirm.wrapper.classList.add('project-entry-create-only');
  const techDecision=document.createElement('p'); techDecision.className='project-entry-hint project-entry-create-only'; techDecision.textContent='Tecnología: PENDIENTE. B-02 crea un Project Shell neutral. El stack y sus dependencias se decidirán después de Architecture mediante una decisión gobernada; este bootstrap no crea frontend/backend/.venv ni manifests tecnológicos.';
  ...
  const grid=document.createElement('div'); grid.className='project-entry-form-grid'; grid.append(mode.wrapper,projectId.wrapper,projectName.wrapper,target.wrapper,businessNeed.wrapper,modelPolicy.wrapper,constraintsConfirm.wrapper,sourceKind.wrapper,source.wrapper);
  ...
  const effects=renderTechnicalDisclosure('Qué puede cambiar esta operación','CREATE_NEW greenfield crea un Project Shell neutral: workspace, Git, metadata y contexto inicial; no fija stack, no crea .venv y no instala dependencias antes de Architecture. OPEN_EXISTING conserva el source y registra metadata; IMPORT_GIT crea una copia local gobernada. Remote Git permanece disabled-by-default.');
  ...
  buttons.append(dry,revalidate); form.append(grid,techDecision,modeHint,hint,effects,buttons);
  ...
  const syncMode=()=>{const isImport=mode.select.value==='IMPORT_GIT';const isCreate=mode.select.value==='CREATE_NEW';sourceKind.wrapper.hidden=!isImport;source.wrapper.hidden=!isImport;sourceKind.select.disabled=!isImport;source.input.disabled=!isImport;for(const el of [businessNeed.wrapper,modelPolicy.wrapper,constraintsConfirm.wrapper,techDecision]) el.hidden=!isCreate;businessNeed.textarea.disabled=!isCreate;modelPolicy.select.disabled=!isCreate;constraintsConfirm.input.disabled=!isCreate;modeHint.textContent=MODE_GUIDANCE[mode.select.value as EntryMode];invalidatePlan();};
  mode.select.addEventListener('change',syncMode);
  for(const input of [projectId.input,projectName.input,target.input,source.input,businessNeed.textarea]) input.addEventListener('input',invalidatePlan);
  sourceKind.select.addEventListener('change',invalidatePlan); modelPolicy.select.addEventListener('change',invalidatePlan); constraintsConfirm.input.addEventListener('change',invalidatePlan);
  syncMode();

  form.addEventListener('submit',async(ev)=>{
    ev.preventDefault();
    const projectIdValue=projectId.input.value.trim(), projectNameValue=projectName.input.value.trim(), targetValue=target.input.value.trim(), sourceValue=source.input.value.trim(), businessNeedValue=businessNeed.textarea.value.trim();
    ...
    if(mode.select.value==='CREATE_NEW'&&businessNeedValue.length<20){status.textContent='BLOCK: describe la necesidad de negocio con al menos 20 caracteres, sin diseñar todavía la solución.';result.replaceChildren();result.dataset.state='error';businessNeed.textarea.focus();return;}
    if(mode.select.value==='CREATE_NEW'&&!constraintsConfirm.input.checked){status.textContent='BLOCK: confirma los constraints del piloto antes de generar el dry-run.';result.replaceChildren();result.dataset.state='error';constraintsConfirm.input.focus();return;}
    ...
    try{
      const intake=buildIntake(mode.select.value as EntryMode,projectIdValue,projectNameValue,targetValue,sourceKind.select.value,sourceValue,businessNeedValue,modelPolicy.select.value,constraintsConfirm.input.checked);lastIntake=intake;
  ...
  if(resumeState){
    ...
    projectId.input.value=String(intake.project_id??'');projectName.input.value=String(intake.project_name??'');target.input.value=String(intake.target_root??'');businessNeed.textarea.value=String(intake.business_need??'');constraintsConfirm.input.checked=Boolean((intake.project_constraints as Record<string,unknown>|undefined)?.local_first);
...
function buildIntake(mode:EntryMode,id:string,name:string,target:string,sourceKind:string,source:string,businessNeed:string,modelPolicy:string,constraintsConfirmed:boolean):Record<string,unknown>{if(mode==='CREATE_NEW'){return{schema_id:'SCHEMA-DEVPL-GSDLC-13-B-PROJECT-INTAKE-V2',schema_version:'2.0',project_id:id.trim(),project_name:name.trim(),project_type:'agent-assisted-sdlc',entry_mode:mode,target_root:target.trim(),business_need:businessNeed.trim(),technology_decision_status:'deferred-to-architecture',stack:{frontend:'undecided',backend:'undecided',database:'undecided'},standards:['MIPSoftware','MIASI'],provider:{mode:'mock',provider_id:'mock'},model_policy:{baseline:modelPolicy,local_model:'optional-opt-in',external_api:'approval-provenance-only'},project_constraints:{local_first:constraintsConfirmed,cloud_required:false,operator_project_writes_allowed:false},restrictions:{arbitrary_shell_allowed:false,silent_network_allowed:false,remote_git_execute_allowed:false}};}const intake:Record<string,unknown>={schema_id:'SCHEMA-DEVPL-GSDLC-03-A-PROJECT-INTAKE-V1',schema_version:'1.0',project_id:id.trim(),project_name:name.trim(),project_type:'agent-assisted-sdlc',entry_mode:mode,target_root:target.trim(),stack:{frontend:'react-typescript',backend:'fastapi-python',database:'sqlite'},standards:['MIPSoftware','MIASI'],provider:{mode:'none',provider_id:null},restrictions:{arbitrary_shell_allowed:false,silent_network_allowed:false,remote_git_execute_allowed:false}};if(mode==='IMPORT_GIT')intake.git_source={kind:sourceKind,location:source.trim()};return intake;}
function textareaField(label:string,id:string,placeholder:string){const wrapper=document.createElement('label');wrapper.className='project-entry-field';wrapper.htmlFor=id;const span=document.createElement('span');span.textContent=label;const textarea=document.createElement('textarea');textarea.id=id;textarea.rows=5;textarea.placeholder=placeholder;textarea.required=true;wrapper.append(span,textarea);return{wrapper,textarea};}
function checkboxField(label:string,id:string,help:string){const wrapper=document.createElement('label');wrapper.className='project-entry-field project-entry-checkbox';wrapper.htmlFor=id;const input=document.createElement('input');input.type='checkbox';input.id=id;input.required=true;const span=document.createElement('span');span.textContent=label;const note=document.createElement('small');note.textContent=help;wrapper.append(input,span,note);return{wrapper,input};}
