...
import { renderAgentEvalTraceView } from '../components/AgentEvalTraceView';
...
interface SettingsState extends SettingsSnapshot {
  ...
  agentEvals?: DevPilotApplicationResponse;
...
export function renderSettingsView(client: DevPilotApiClient, token: () => string): HTMLElement {
  ...
  async function refresh(): Promise<void> {
    ...
    await runBounded<DevPilotApplicationResponse<any>>([
      ...
      { key: 'agentEvals', run: () => fresh.settingsAgentEvals() },
      ...
      if (result.value) {
        ...
        if (result.key === 'agentEvals') state.agentEvals = result.value;
        if (result.key === 'portfolio') state.portfolio = result.value;
      }
      if (result.error) state.errors[result.key] = result.error;
      draw();
    });
    const responseKeys = ['workspace', 'providers', 'policy', 'securityPosture', 'modelGateway', 'agentRuntime', 'ragContext', 'agentExecution', 'agentEvals', 'portfolio'];
    const responses = [state.workspace, state.providers, state.policy, state.securityPosture, state.modelGateway, state.agentRuntime, state.ragContext, state.agentExecution, state.agentEvals, state.portfolio].filter(Boolean);
  ...
  function draw(): void {
    ...
    aiControlCenter.innerHTML = renderAIControlCenterShell({
      ...
      agentEvalHtml: renderAgentEvalTraceView(state.agentEvals?.data),
