...
const required=[
  ...
  ['action-guide',view.includes('renderPreCodeActionGuide')&&view.includes('selectAuthoringMode')],
  ['manual-import',view.includes('Modo de autoría')&&view.includes('Importar archivo local')&&view.includes('data-pre-code-authoring-file')&&view.includes('selectAuthoringMode')],
  ['draft-first',view.includes('Generar propuesta con DevPilot')&&view.includes('Propuesta completa generada como DRAFT')],
  ['decision-inbox',view.includes('Decisiones pendientes')&&view.includes('Guardar decisiones y regenerar propuesta')],
  ['advanced-semantic-detail',view.includes('Ver análisis de DevPilot')],
  ...
  ['typed-contract',types.includes('interface PreCodeWizardStage')&&types.includes("format: 'unified'")&&types.includes('required_stage?')],
...
console.log(JSON.stringify({status:'PASS',checks:required.length,route:'/pre-code',authority:'server-side',draft_first:true,full_regression_runs:0},null,2));
